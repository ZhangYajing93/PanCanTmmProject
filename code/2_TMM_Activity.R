...
load('/pub5/xiaoyun/Jobs/J22/ZYJ2020/Telomere/OfficialVersion/ResourceData/probeTMMOrder.RData')
...
#####################
TMMgsvaAllGene<-read.table('E:/0/telomeres/Results/OfficialVersion/gsva/CaseCtrl/TMMgsvaAllGene.txt',
...
p<-ggviolin(data = subset(TMMgsvaAllGene, tissue_type2 %in% c('tumor', 'normal')), nrow=1,
            x = 'tissue_type2', y = 'difTMM', facet.by = 'cancer_type',
            palette = 'nc',draw_quantiles = c(0.25,0.5,0.75),font.label = list(size=6),
            fill = 'tissue_type2', add = c("jitter"),add.params = list(size=0.2,alpha=0.5)) + 
  theme_classic()+
  theme(aspect.ratio = 1)+
  stat_compare_means(comparisons = list(c('tumor','normal')),label= 'p.signif',method='wilcox.test') # 不知如何解释，不使用
ggpar(p,legend = 'top')

p1<-ggviolin(data = subset(TMMgsvaAllGene, tissue_type2 %in% c('tumor', 'normal')), nrow=1,
             x = 'tissue_type2', y = 'Telomerase_difTMMway', facet.by = 'cancer_type',
             palette = 'nc',draw_quantiles = c(0.25,0.5,0.75),font.label = list(size=6),
             fill = 'tissue_type2', add = c("jitter"),add.params = list(size=0.2,alpha=0.5)) + 
  theme_classic()+
  theme(aspect.ratio = 1)+
  stat_compare_means(comparisons = list(c('tumor','normal')),label= 'p.signif',method='wilcox.test') # sig
p2<-ggviolin(data = subset(TMMgsvaAllGene, tissue_type2 %in% c('tumor', 'normal')), 
             x = 'tissue_type2', y = 'ALT_difTMMway', facet.by = 'cancer_type',nrow=1,
             palette = 'nc',draw_quantiles = c(0.25,0.5,0.75),font.label = list(size=6),
             fill = 'tissue_type2', add = c("jitter"),add.params = list(size=0.2,alpha=0.5)) + 
  theme_classic()+
  theme(aspect.ratio = 1)+
